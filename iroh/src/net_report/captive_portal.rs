...
/// Errors from the captive portal detection check.
#[cfg(not(wasm_browser))]
#[stack_error(derive, add_meta)]
#[non_exhaustive]
pub(super) enum CaptivePortalError {
    /// DNS resolution for the relay hostname failed.
    #[error(transparent)]
    DnsLookup {
        #[error(from)]
        source: StaggeredError<DnsError>,
    },
    /// Could not construct the HTTP client (TLS or system error).
    #[error("Creating HTTP client failed")]
    CreateReqwestClient {
        #[error(std_err)]
        source: reqwest::Error,
    },
    /// The HTTP request itself failed (network error, connection refused, etc.).
...
/// Detects whether the network has a captive portal intercepting HTTP traffic.
///
/// Sends an HTTP request to a relay's `/generate_204` endpoint with a
/// challenge header. A well-behaved relay responds with 204 and the
/// expected response header. Any other status or a missing/wrong header
/// indicates a captive portal is present. Returns `true` when a portal
/// is detected.
#[cfg(not(wasm_browser))]
