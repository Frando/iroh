...
/// Errors from HTTPS probe execution in the actor's probe pipeline.
#[allow(missing_docs)]
#[stack_error(derive, add_meta)]
#[non_exhaustive]
pub(super) enum ProbesError {
    /// The underlying HTTP request or DNS resolution failed.
    #[error("HTTPS probe failed")]
    ProbeFailure { source: MeasureHttpsLatencyError },
    /// The probe was cancelled before it could complete.
    #[error("Probe cancelled")]
    Cancelled,
    /// The probe exceeded its per-probe timeout.
    #[error("Probe timed out")]
    Timeout,
}

/// Result of a successful HTTPS latency probe against a single relay.
#[derive(Debug, Clone)]
pub(super) struct HttpsProbeReport {
    /// URL of the relay that was probed.
    pub(super) relay: RelayUrl,
    /// Round-trip latency measured from HTTP request to first response byte.
...
/// Executes an HTTPS latency probe against a single relay.
///
/// Sends a GET request to the relay's probe endpoint and measures the
/// round-trip time. DNS resolution uses the provided resolver (on native
/// targets) rather than the system resolver, so it follows the same path
/// as relay client connections.
