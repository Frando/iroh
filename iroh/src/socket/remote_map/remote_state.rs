...
use noq::{Closed, WeakConnectionHandle};
...
pub use self::{
    path_watcher::{Path, PathEvent, PathEventStream, Paths},
...
impl RemoteStateActor {
    ...
    fn handle_connection_close(&mut self, conn_id: ConnId, closed: Closed) {
        ...
        if let Some(conn_state) = self.connections.remove(&conn_id) {
            ...
            conn_state.path_store.close(closed);
        }
    ...
    fn handle_path_event(&mut self, conn_id: ConnId, event: Result<NoqPathEvent, noq::Lagged>) {
        ...
        match event {
            ...
            NoqPathEvent::Discarded { id, path_stats } => {
                ...
                // The Abandoned event was already processed (or is about to be);
                // nothing more to publish here. We already emitted a Closed event
                // from remove_path with the last stats we could fetch.
            }
        ...
        if let Some((addr, rtt)) = selected_path {
            ...
            for conn in self.connections.values() {
                let value = conn
                    .paths
                    .iter()
                    .find(|(_path_id, path_addr)| **path_addr == addr)
                    .map(|(id, addr)| (*id, addr.clone().into()));
                conn.path_store.record_selected(value);
            }
        } else {
...
struct ConnectionState {
    ...
    handle: WeakConnectionHandle,
    /// The path observation store we publish to users for this connection.
    path_store: PathStateSender,
...
impl ConnectionState {
    ...
    fn add_open_path(
            ...
            && let Some(path) = conn.path(path_id)
        {
            ...
            self.path_store.record_opened(handle, remote.into());
        }
    }

    /// Removes a path from this connection and records it as closed in the
    /// [`PathStore`] with the provided final statistics.
    fn remove_path(
        &mut self,
        path_id: &PathId,
        conn: &noq::Connection,
        ...
        self.path_store
            .record_closed(*path_id, addr.clone().into(), conn);
        Some(addr)
    }
}
